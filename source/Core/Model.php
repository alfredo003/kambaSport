...
abstract class Model
{
    ...
    public function findByIDcode(int $id, string $columns = "*"): ?Model
    {
        $find = $this->find("IDcode = :id", "id={$id}", $columns);
        return $find->fetch();
    }

    public function findmax(?string $terms = null, ?string $params = null, string $columns = "*")
    {
        if ($terms) {
            $this->query = "SELECT MAX(id) as id FROM {$this->entity} WHERE {$terms}";
            parse_str($params, $this->params);
            return $this;
        }

        $this->query = "SELECT MAX(id) as id FROM " . $this->entity;
        return $this;
    }
